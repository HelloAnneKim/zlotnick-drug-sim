...
init_subunit_con = 2*10.^-6;
m = AddState(m, 'n1', 'Solution', init_subunit_con); % Molar (ref 1999 Zlotnick)
m = AddStatesWithSameConcentration(m, drugless_capsids(1,2:12), 'Solution', 0);

%% Add States of Drugged Capsids
m = AddSeed(m, 'd1_0', 0)
m = AddState(m, 'd1', 'Solution', 'd1_0'); % Molar 
...
m = AddDrugReactionsN(m, '', 12, 'k_felong', []);

m = FinalizeModel(m);

%% Construct experiment
concentration1 = 0;
concentration2 = 1*10^-7;
concentration3 = 2*10^-7;
concentration4 = 3*10^-7;
concentration5 = 4*10^-7;
concentrations = {concentration1, concentration2, concentration3, concentration4, concentration5};
legend_concentrations = {num2str(concentration1), num2str(concentration2), num2str(concentration3), num2str(concentration4), num2str(concentration5)};

figure
hold on
times = linspace(0, tF, 100);
% indices_of_output = [1,13,15,91,104];
indices_of_output = [13];
for i = 1:size(concentrations,2)
    ...
    %plot for raw concentrations
    plot(times, sim1.y(times, indices_of_output)) 
    
    %plot for efficacy
    %plot(times, sim1.y(times, indices_of_output)./init_subunit_con);
    
    
end

legend(legend_concentrations);
xlabel('Time')
ylabel('Subunits in Complete Viral Capsids')
ylabel('Efficacy (coplete capsids/potential capsids)')
title('Drugging Zlotnick Model with various drug concentrations');
