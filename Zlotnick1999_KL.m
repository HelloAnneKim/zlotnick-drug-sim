...
%% Parameters
k_fslow = 10.^6; % M^-1*s^-1 (ref 1999 Zlotnick) dimer and trimer formation
m = AddParameter(m, 'k_fslow', k_fslow);
k_f = 10.^8; % M^-1*s^-1 (ref 1999 Zlotnick)
m = AddParameter(m, 'k_f', k_f);
k_aContact = 10.^4; % M^-1 (ref 1999 Zlotnick)
...
k_b2 = k_f./k_a2; % s^-1 (ref 1999 Zlotnick)
...
m = AddReaction(m, '', {'n1', 'n1'}, {'n2'}, 'k_f', 'k_b2'); % n1+n1 -> n2 w/ k_f forward and k_b2 backwards
...
xlabel('Time (Seconds)')
ylabel('Amount (Molar)')
title('Zlotnick 1999: KL; slow nucleation f/b finagled');